 ...
 *   or otherwise
 ******************************************************************************/
sl_status_t mod_som_calendar_set_time_f(uint16_t year, uint8_t month,
                                        uint8_t month_day, uint8_t hour, \
...
//                                   year,month,month_day,hour,min,sec,tz_offset);
  ...
  sl_status_t status=sl_sleeptimer_build_datetime(
    ...
    // initialization of the start date
     ...
     sl_sleeptimer_timestamp_t time = 0u;//,time_1970=0u;
...
//     }
     ...
     err_code = sl_sleeptimer_convert_date_to_time(&adate, &time);
